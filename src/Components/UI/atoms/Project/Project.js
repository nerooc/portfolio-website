...
const Project = props => {
  ...
  const buttonDemo = props.tags.includes('coded') ? <Button onClick={() => {
    window.open(props.demolink);
  }} white noTransition small>DEMO</Button> : null;
  const buttonGithub = props.tags.includes('coded') ? <Button onClick={() => {
    window.open(props.ghlink);
  }} white noTransition small>GITHUB</Button> : null;
  const buttonFigma = props.tags.includes('designed') ? <Button onClick={() => {
    window.open(props.figmalink);
  }} white noTransition small>FIGMA</Button> : null;
